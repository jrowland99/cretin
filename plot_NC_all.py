...
cretin_dir      = '/home/jeff/Research/Export_Control/Cretin/Neon_Cell/td_latest/sw36_off/'#'/home/jeff/Research/Export_Control/Cretin/Neon_Cell/rad_trans_grid_v2/'
cretin_ss_dir   = '/home/jeff/Research/Export_Control/Cretin/Neon_Cell/steady_state_latest/sw36_off/'
...
for i, gen in enumerate(gens):
    ...
    for j, p in enumerate(press):
        ...
        # Cretin SS
        #--------------------------------------------------------------------------
        cretin_plt = os.path.join(cretin_ss_dir,gen,str(pressure)+'torr','ne_cell.plt')
        cretin_obj = CretinData(cretin_plt)
        cretin_obj.load_data()
        cretin_data = cretin_obj.get_data()
        ax.plot(cretin_data['time']-offset,cretin_data['te'],label = 'Cretin SS',linestyle='dotted')
        #--------------------------------------------------------------------------
...
#%% Plot Temperatures for cretin only
gens = ['gen1c']
directory       ='/home/jeff/Research/Code_Projects/Cloudy_Projects/Z_machine/Neon_cell/Ne_cell_steady_state/all_gen/'
...
cretin_ss_dir   = '/home/jeff/Research/Export_Control/Cretin/Neon_Cell/steady_state_latest/'
helios_dir      = '/home/jeff/Research/Code_Projects/Cloudy_Projects/Z_machine/Neon_cell/helios_data'
fig, axes       = plt.subplots(3, 3,sharey='row')
plotPeak        = True
offset          = 100
for i, gen in enumerate(gens):
    ...
    for j, p in enumerate(press):
        ...
        if j == 2:
            ...
            ax_twin.set_ylabel('Temperature [eV]')
            #ax_twin.tick_params(axis='y')
    plt.suptitle('Cretin w/ continuum radiation transport')
    if i ==1 :
        ax.set_ylim([0,120])
        if j == 2:
            yticks      = ax.get_yticks()
...
if plotPeak:
    # plt.savefig(os.path.join(cretin_dir,'all_gen_Te_peak'+ext), transparent=False)
    plt.savefig(os.path.join(cretin_dir,'cretin_td_ss_sw36_on'+ext), transparent=False)
else:
    plt.savefig(os.path.join(cretin_dir,'all_gen_temperatures'+ext), transparent=False)
#%% Plot Temperatures for all, two cretin results
directory='/home/jeff/Research/Code_Projects/Cloudy_Projects/Z_machine/Neon_cell/Ne_cell_steady_state/all_gen/'
cretin_dir = '/home/jeff/Research/Export_Control/Cretin/Neon_Cell/sw36_off/'
cretin_dir2 = '/home/jeff/Research/Export_Control/Cretin/Neon_Cell/rad_trans_grid_v2/'
helios_dir = '/home/jeff/Research/Code_Projects/Cloudy_Projects/Z_machine/Neon_cell/helios_data'
fig, axes = plt.subplots(3, 3,sharey='row')
plotPeak = True
offset = 100
for i, gen in enumerate(gens):
    ...
    for j, p in enumerate(press):
        ...
        cretin_obj.load_data()
        cretin_data = cretin_obj.get_data()
        ax.plot(cretin_data['time']-offset,cretin_data['te'],label = 'Cretin switch 36 OFF')
        #--------------------------------------------------------------------------
        cretin_plt = os.path.join(cretin_dir2,gen,str(pressure)+'torr','ne_cell.plt')
        cretin_obj = CretinData(cretin_plt)
        cretin_obj.load_data()
        cretin_data = cretin_obj.get_data()
        ax.plot(cretin_data['time']-offset,cretin_data['te'],label = 'Cretin switch 36 ON',color='navy')
        #--------------------------------------------------------------------------
...
# plt.savefig(os.path.join(directory,'all_gen_temperatures'+ext), transparent=False)
if plotPeak:
    plt.savefig(os.path.join(cretin_dir,'all_gen_Te_peak_cretin_comp'+ext), transparent=False)
else:
    plt.savefig(os.path.join(cretin_dir,'all_gen_temperatures_cretin_comp'+ext), transparent=False)
    
#%% Plot time dependent results of cloudy, cretin, helios for gen1c
...
for i, gen in enumerate(gens):
    ...
    for j, p in enumerate(press):
        ...
        #--------------------------------------------------------------------------
        ctd_df = ctdp[pressure]
        ax.plot(ctd_df['time']-offset,ctd_df['te'],label = 'Cloudy',color='goldenrod')
        ax.fill_between(ctd_df['time']-offset,ctd_df['te'],ctd_df['te']-ctd_df['te_std'],alpha=0.3,color='grey', edgecolor='none')
        ax.fill_between(ctd_df['time']-offset,ctd_df['te'],ctd_df['te']+ctd_df['te_std'],alpha=0.3,color='grey', edgecolor='none')
        #--------------------------------------------------------------------------
        # Helios
        ...
        cretin_obj.load_data()
        cretin_data = cretin_obj.get_data()
        ax.plot(cretin_data['time']-offset,cretin_data['te'],label = 'Cretin')
        #--------------------------------------------------------------------------
        # Cretin SS
        #--------------------------------------------------------------------------
        # cretin_plt = os.path.join(cretin_ss_dir,gen,str(pressure)+'torr','ne_cell.plt')
        ...
        if j == 2:
            ...
            ax_twin.set_ylabel('Temperature [eV]')
            #ax_twin.tick_params(axis='y')
    plt.suptitle('Time-dependent Atomic Kinetics Temperature Results')
    if i ==1 :
        ax.set_ylim([0,120])
        if j == 2:
            yticks      = ax.get_yticks()
...
if plotPeak:
    # plt.savefig(os.path.join(cretin_dir,'all_gen_Te_peak'+ext), transparent=False)
    plt.savefig(os.path.join(cretin_dir,'gen1c_td_te_peak'+ext), transparent=False)
else:
    plt.savefig(os.path.join(cretin_dir,'gen1c_td_te'+ext), transparent=False)
#%% Compare two cretin temperatures
plt.rcParams["figure.dpi"] = 350
plt.rcParams["lines.linewidth"] = 1
plt.rcParams.update({"font.size": 10})
plt.rcParams.update({"legend.fontsize": 11})
...
for i, gen in enumerate(gens):
    ...
    for j, p in enumerate(press):
        ...
        cretin_obj.load_data()
        cretin_data = cretin_obj.get_data()
        ax.plot(cretin_data['time']-offset,cretin_data['te'],label = 'Cretin Lines',color = 'k')
        # #--------------------------------------------------------------------------
        # plot_exp(gen,pressure,fig,ax)
        plot_2020_exp(gen,pressure,fig,ax)
        ax.set_ylim([0,40])
...
if plotPeak:
    # plt.savefig(os.path.join(cretin_dir,'all_gen_Te_peak'+ext), transparent=False)
    plt.savefig(os.path.join(cretin_ss_dir,'gen1c_te_lines_no_term_peak'+ext), transparent=False)
else:
    plt.savefig(os.path.join(cretin_ss_dir,'gen1c_te_lines_no_term'+ext), transparent=False)